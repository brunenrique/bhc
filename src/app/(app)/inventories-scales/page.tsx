...
export default function InventoriesScalesPage() {
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-2">
          <ClipboardList className="h-7 w-7 text-primary" />
          <h1 className="text-3xl font-headline font-bold">Inventários e Escalas</h1>
        </div>
      ...
      <Card className="shadow-sm">
        <CardHeader>
          <CardTitle className="font-headline">Gerenciar Inventários e Escalas</CardTitle>
          <CardDescription>Acompanhe formulários de inventários/escalas personalizados e seus status de preenchimento.</CardDescription>
          <div className="flex flex-col sm:flex-row gap-2 pt-4">
            <div className="relative flex-1">
              <Search className="absolute left-2.5 top-3 h-4 w-4 text-muted-foreground" />
              <Input placeholder="Buscar inventários/escalas..." className="pl-8" />
            </div>
        ...
        <CardContent>
          {mockAssessments.length > 0 ? (
            ...
            <div className="text-center py-10">
              ...
              <h3 className="mt-2 text-sm font-medium text-foreground">Nenhum inventário/escala encontrado</h3>
              <p className="mt-1 text-sm text-muted-foreground">Comece criando um modelo ou atribuindo um a um paciente.</p>
            </div>
          )}
        </CardContent>
      </Card>
