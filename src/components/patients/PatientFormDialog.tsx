...
const initialFormState: Partial<Patient> = {
  ...
  dateOfBirth: "",
  address: "",
  sessionNotes: "", // Placeholder for encrypted notes UI
};
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          <div>
            <Label htmlFor="sessionNotes">Histórico / Anotações Cifradas (Exemplo)</Label>
            <Textarea 
              id="sessionNotes" 
              name="sessionNotes" 
              placeholder="Anotações importantes sobre o paciente... (serão cifradas)" 
              value={formData.sessionNotes || ''} 
              onChange={handleChange}
              rows={4} 
            />
            <p className="text-xs text-muted-foreground mt-1">Estas notas serão armazenadas de forma segura.</p>
          </div>
